...
export default function ConsentCheckboxes() {
  ...
  const labelLicitOrigin =
    "Declaro que as quantias doadas não são produto de crime ou oriundas de quaisquer atividades ilícitas.";

  const errorBoth =
    "Por favor, leia nossos termos de uso e confirme que a origem da doação é lícita.";
  const errorPrivacyTermsAck = "Por favor, leia nossos termos de uso.";
  const errorLicitOrigin =
    "Por favor, confirme que a origem da doação é lícita.";
        ...
        <FormHelperText>
          {donation.consent.error
            ? !donation.consent.consentLicitOrigin &&
              !donation.consent.privacyTermsAck
              ? errorBoth
              : !donation.consent.consentLicitOrigin
              ? errorLicitOrigin
